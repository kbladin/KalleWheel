// External code is included here in the same file due to the lack of support for multiple jsx files in CEP
// Order of content:
// stdlib.js (from xlib)
// ColorSwatches.jsx (from xlib)
// Json.js
// KalleWheel.js
...
//
// $Id: Stream.js,v 1.31 2014/11/25 04:11:22 anonymous Exp $
...
function setBackgroundColorLAB(l, a, b) {
  var LAB = new LabColor();
  LAB.l = l;
  LAB.a = a;
  LAB.b = b;
...
function getBackgroundColorLAB() {
    var json = "{\"l\":\"" + app.backgroundColor.lab.l +
             "\", \"a\":\"" + app.backgroundColor.lab.a +
             "\", \"b\":\"" + app.backgroundColor.lab.b + "\"}";
    json = '/*' + json + '*/'; // Wrap the JSON
    sendEvent("backgroundColorImported", json);
};
...
function sendSwatches(sw) {
    var json = "[";
    for (var index = 0; index < sw.colors.length; index++) {
        json += "{\"l\":\"" + sw.colors[index].color.lab.l +
        "\", \"a\":\"" + sw.colors[index].color.lab.a +
        "\", \"b\":\"" + sw.colors[index].color.lab.b + "\"}";
        if (index != sw.colors.length - 1) {
            json += ",";
        }
    }
    json += "]";
    json = '/*' + json + '*/'; // Wrap the JSON
    sendEvent("swatchesImported", json);
}

var xLib;
try {
	xLib = new ExternalObject("lib:\PlugPlugExternalObject");
} catch(e) { alert("KalleWheel Warning: Some features that require version 15.0 (CC 2014) are missing. This may cause the extension to behave unexpectedly. " + e); }
...
function loadSwatches(nColors) {
    var sw = new ColorSwatches();

    var filePath = app.preferencesFolder + "/KalleWheelSwatches.aco"
    var file = File(filePath);
    if (file.open("r")) {
        file.close();
        sw.readFromACOFile(filePath);
    }
    // Fill in if colors are missing
    for (var index = sw.colors.length; index < nColors; index++) {
        var LAB = new LabColor();
        LAB.l = 50;
        LAB.a = 0;
        LAB.b = 0;
        var sColor = new SolidColor();
        sColor.lab = LAB;
        var nm = "KalleWheelColor" + index;
        sw.addColor(sColor, nm);
    }
    
    return sw;
}

function saveSwatches(colorsJson) {
    var colors = JSON.parse(colorsJson);
    var sw = new ColorSwatches();

    for (var index = 0; index < colors.length; index++) {
        var LAB = new LabColor();
        LAB.l = colors[index].l;
        LAB.a = colors[index].a;
        LAB.b = colors[index].b;
        var sColor = new SolidColor();
        sColor.lab = LAB;
        var nm = "KalleWheelColor" + index;
        sw.addColor(sColor, nm);
    ...
    var filePath = app.preferencesFolder + "/KalleWheelSwatches.aco"
    sw.writeToACOFile(filePath);
}

function readGamutMask() {
  var filePath = app.preferencesFolder + "/KalleWheelGamutMask.json"
  var file = File(filePath);
  if (file.open("r")) {
    var jsonStr = file.read();
    file.close();
    jsonStr = '/*' + jsonStr + '*/'; // Wrap the JSON
    sendEvent("readGamutMask", jsonStr);
  }
}

function writeGamutMask(jsonStr) {
  var filePath = app.preferencesFolder + "/KalleWheelGamutMask.json"
  var file = File(filePath);
  if (file.open("w")) {
    file.write(jsonStr);
...
function initialize(nColors) {
    var sw = loadSwatches(nColors);
    sendSwatches(sw);
    getBackgroundColorLAB();
    readGamutMask();
}
